...
const BookingScreen = () => {
    ...
    const { error, loading, data: room = [] } = useFetchData(() => getRoomByID(roomid), [roomid]);
    const { data: inventoryByID = [] } = useFetchData(() => getInventoryByID(room?.inventoryID), [room]);
    const { data: facilitiesByID = [] } = useFetchData(() => getFacilitiesById(room?.facilitiesID), [room]);
    const [totalAmount, setTotalAmount] = useState(1);
    const [numberOfPeople, setNumberOfPeople] = useState(1); // State to store the number of people for booking
    const currentUser=JSON.parse(localStorage.getItem('currentUser'));


    console.log(facilitiesByID)
            ...
            <div className="container ">
                <div className="row justify-content-center">
                    <div className="col-md-8 border p-4 rounded">
                        ...
                        <div className="room-details">
                            ...
                            <p><strong>Inventory Details: </strong> {inventoryByID.quantity} { inventoryByID.name}</p>
                            <p><strong>Facilities: </strong> {facilitiesByID.quantity} { facilitiesByID.name}</p>
                ...
                <div className="row justify-content-center mt-4">
                    <div className="col-md-8 border p-4 rounded">
                        ...
                        <p><strong>Total Amount:</strong> {totalAmount} RON
                        {totalDays >= 10 && <p className='text-success'><strong> 5% discount! </strong></p> }
                        </p>
